...
public class PublishBlogPostStep : KernelProcessStep<BlogPublishingResponse>
{
    ...
    private readonly ILogger<PublishBlogPostStep> _logger;

    public PublishBlogPostStep(ILogger<PublishBlogPostStep> logger)
    {
        _logger = logger;
    }
        ...
        KernelProcessStepContext context)
    {
        ...
        try
        {
            // Get repository from MCPConfig
            var githubRepo = MCPConfig.GitHubRepo;
            ...
            var instructions = $"Create a new markdown file in the repository with the following content:\n" +
                  $"Repository: {githubRepo}\n" +
                  $"Filename: blog-posts/{DateTime.Now:yyyy-MM-dd}-blog-post.md\n" +
